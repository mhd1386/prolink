...
def get_installation_directory():
    ...
    # Default directory for shared hosting
    default_dir = "/public_html"
    
    print_colored(f"Default installation directory for shared hosting: {default_dir}", 'yellow')
    print_colored("This is where your domain points to.", 'blue')
...
def create_virtual_env():
    ...
    # Check if we're in cPanel Python app (has existing virtualenv)
    cpanel_venv_path = Path("/home/mihanexp/virtualenv/prolink/3.9/bin/activate")
    if cpanel_venv_path.exists():
        print_colored("⚠️ cPanel Python app detected - using existing virtual environment", 'yellow')
        print_colored("   Virtual environment path: /home/mihanexp/virtualenv/prolink/3.9", 'blue')
        return False  # Don't create new venv, use cPanel's
    ...
    try:
        print_colored("Creating virtual environment with python -m venv venv", 'blue')
        ...
        print_colored("Upgrading pip in virtual environment...", 'blue')
        ...
        print_colored("This might be due to:", 'yellow')
        print_colored("1. Missing venv module (install python3-venv)", 'blue')
        print_colored("2. Permission issues", 'blue')
        print_colored("3. cPanel Python app environment", 'blue')
        print_colored("⚠️ Continuing without virtual environment", 'yellow')
        return False
    except Exception as e:
        print_colored(f"❌ Unexpected error creating virtual environment: {e}", 'red')
        print_colored("⚠️ Continuing without virtual environment", 'yellow')
        return False
